...
export const decidePressure = ({ bytesMB = 0, budgetMB = 1800, driftMs = 0, heapMB = 0 } = {}) => {
    ...
    // Timer drift is the device-relative swap signal: it fires when THIS device
    // is starving regardless of absolute bytes (a small device near its limit).
    if (driftMs > 5000) level = Math.max(level, 2)
    else if (driftMs > 2500) level = Math.max(level, 1)
