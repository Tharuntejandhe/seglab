 ...
 *
 * lib/ and models/ are gitignored (~90 MB). Idempotent: complete files are
 * skipped. Fails loudly if the transformers→ORT version pin ever drifts.
 *
...
const jobs = [
    ...
    ...SLIMSAM_FILES.map((entry) => {
        const file = typeof entry === 'string' ? entry : entry.file
        return {
            url: `https://huggingface.co/${SLIMSAM}/resolve/main/${file}`,
            dest: `models/${SLIMSAM}/${file}`,
